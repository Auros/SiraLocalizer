param(
    [parameter(Position = 0)] $ManifestPath
)
...
if (!(Test-Path $ManifestPath)) {
    Write-Host "File '$ManifestPath' does not exist" -ForegroundColor Red
    exit(-1)
}

$manifest_content = Get-Content $ManifestPath | ConvertFrom-Json
...
$manifest_content | ConvertTo-Json | Set-Content $ManifestPath
